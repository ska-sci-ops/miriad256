...
c    pjt   25nov03      fix problems in headcp if blc/trc are sub-
c                       imaged
...
c
      ...
      call keyr('medsmooth',medsmooth,0.0)
...
c
      if (dout) then
        ...
        enddo
        call headcp(lin,lout,naxis,axnum,blc,trc)
        call boxinfo(boxes,maxnax,blc,trc)
      endif
...
c
      do i = 1, naxis
        ...
        call rdhdr(lin,'crpix'//cin,crpix(i),0.0)
        call rdhdr(lin,'cdelt'//cin,cdelt(i),0.0)
        if (i.le.2) then
...
c
      do i = 1, 2
        if (cdelt(i).lt.0.0) center(i) = -center(i)
        cdelt(i) = abs(cdelt(i)*rts)
      enddo
      if (rmax.eq.0.0) rmax = nsize(1)*cdelt(1)
      if (rstep.eq.0.0) rstep = cdelt(1)
