...
c+
c     TVFLAG is a MIRIAD task which allows interactive baseline
...
c
c     If a menu program is active, then TVFLAG will attempt
...
c    jm    07may90    Changed Dispedit to include declared size of
c                     iflag.
...
c                     from 7 to 10.  This array is defined in tvflag.h.
...
c    rjs    4mar93    Fudges to get it to work on data out of time
c                     order.
c    nebk  09mar94    Declare APRI long enough to avoid string
c                     truncation warning on convex (does not affect run
c                     time performance)
c    rjs   27jul94    Get rid of some debugging write statements.
c    rjs   11nov94    Eliminate bounds violation for large numbers of
c                     times.
...
c***********************************************************************
c  LdMenu -- Internal routine to load tvflag.h common block.
c
...
c  by TVFLAG and provides a listing of the accepted commands allowed
c  by the program.
c
c  Input:
...
c
      ...
      integer param(5)
c
c  This routine is used by TVFLAG to return a command and its
...
c
c
c  This routine is used by TVFLAG to return a selected command and
...
c
      ...
      do while (cmd .eq. iEDIT(iNULL))
        j = iNULL
        k = 1
        call Prompt(string, length, 'TVFLAG> ')
...
c
c  This routine is used by TVFLAG to alter the input flag array "iflag"
...
c  array.  This routine is used just by TVFLAG.
c
c  Input:
...
c
c  This routine is currently used just by TVFLAG.
c
c  Input:
