...
#define VERSION "doc2man: Revision 1.3, 2010/04/28"
...
private void usage()
{
  fprintf(stderr,
    "%s\n"
    "Reformat a Miriad .doc file into a man page.\n\n"
    "Usage:\n"
    "doc2man [-?] [-section] infile > outfile\n"
    "     (man) section must start with a digit. [Default: 1]\n",
    VERSION);
}
...
----------------------------------------------------------------------*/
{
  ...
  while (gline(line,MAXLINE,fin) != NULL) {
    /* If the line does not start with a percent, just write it out.
    ...
    if (*line != '%') {
      ...
      case 'N':
        ...
        if (has_file) {
          /* Patch the string locally... */
          *has_file++ = 0;

          /* ...but check next character. */
          if (*has_file != 'F') {
